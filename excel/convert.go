...
// ToBool 字符串中将"1", "t", "T", "true", "TRUE", "True"转为true
...
// ToInt8 string会按顺序尝试将数据解析为int64\uint64\float64\bool，然后再转换为int8
// float会抹去小数
// uint8~64、int8~64都会做默认的转换
// bool类型的数据，true-1；false-0
...
// ToUint16 尽最大努力将一个值转为uint16类型的数据
...
// ToUint32 尽最大努力将一个值转为uint32类型的数据
...
// ToUint64 尽最大努力将一个值转为uint64类型的数据
...
// ToFloat32 尽最大努力将一个值转为float32类型的数据
...
// ToFloat64 尽最大努力将一个值转为float64类型的数据
// string会按顺序尝试将数据解析为float64\uint64\bool，然后再转换为float64
// uint8~64、int8~64都会做默认的转换
// bool类型的数据，true-1；false-0
